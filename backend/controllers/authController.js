...
export const verifyOtp = async (req, res) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ message: "User verified and registered successfully", user });
  } catch (err) {
...
export const updateProfile = async (req, res) => {
  const { name, age, course, college, passingYear } = req.body;
  const userId = req.user.userId; // Assumes middleware sets this

  try {
    // Optional: Add validation for email, passingYear, etc.
    const updatedFields = {
      ...(name && { name }),
      ...(age && { age }),
      ...(course && { course }),
